...
class Database {
    ...
    // External food logging methods
    async logExternalFood(userId, externalFoodId, name, quantity, unit, calories, nutritionData, logDate) {
        const {
            calories_per_100g,
            protein_per_100g,
            carbs_per_100g,
        ...
        const sql = `
            INSERT INTO food_logs (
                user_id, external_food_id, name, quantity, unit, calories, log_date,
                calories_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, 
                fiber_per_100g, brand, external_source_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 
                (SELECT id FROM external_food_sources WHERE name = ? LIMIT 1)
            )
        `;
        return await this.query(sql, [
            userId, externalFoodId, name, quantity, unit, calories, logDate,
            calories_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g,
            fiber_per_100g, brand, source || 'Open Food Facts'
        ]);
    }

    // Cache external foods for performance
    async cacheExternalFood(externalId, sourceId, name, nutritionData) {
        const {
            calories_per_100g,
            protein_per_100g,
            carbs_per_100g,
        ...
        `;
        return await this.query(sql, [
            externalId, sourceId, name, calories_per_100g,
            protein_per_100g, carbs_per_100g, fat_per_100g, fiber_per_100g,
            brand, countries
        ]);
    }

    // Get cached external foods
    async getCachedExternalFoods(searchTerm, limit = 10) {
        const sql = `
            ...
            WHERE cef.name LIKE ? AND efs.is_active = TRUE
    ...
    // Get external food sources
    async getExternalFoodSource(sourceName) {
        const sql = 'SELECT * FROM external_food_sources WHERE name = ? AND is_active = TRUE';
        const results = await this.query(sql, [sourceName]);
        return results[0];
    }
}
